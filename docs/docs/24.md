...
assignment.
  ...
  - `$auto_indent`  
    Contains the current preference for auto indent. Can be "off", "on",
    or "smart".
  - `$calltip_ID`  
  ...
  - `$em_tab_dist`  
    If tab stop emulation is turned on in the Tab Stops... dialog of the
    Preferences menu, the value is the distance between emulated tab
    stops. If tab emulation is turned off, the value is 0.
  - `$empty_array`  
  ...
  - `$tab_dist`  
    ...
    in the Tab Stops... dialog of the Preferences menu.
  - `$text_length`  
  ...
  - `$use_tabs`  
    ...
    emulated tabs" button in the Tab Stops... dialog of the Preferences
    menu.)
  - `$wrap_margin`  
...
## Built-in Subroutines
  ...
  - `filename_dialog( [title[, mode[, defaultPath[, filter[, defaultName]]]]] )`  
    ...
    "Choose file". If `<mode>` is set to "exist" (default), the "New
    File Name"TextField of the FSB will be unmanaged. If "new", the
    TextField will be managed. `<defaultPath>` is the default path to
  ...
  - `read_file( filename )`  
    ...
    1 in $read\_status, and the contents of the file as a string in the
  ...
  - `shell_command( command, input_string )`  
    Executes a shell command, feeding it input from input\_string. On
  ...
  - `string_compare(string1, string2 [, consider-case])`  
    ...
    less than string2 or 1 if string1 is greater than string2. The
    values for the optional consider-case argument is either "case" or
    "nocase". The default is to do a case sensitive comparison.
...
should not use any of these functions in any new macro you write. Among
these are all action routines with hyphens in their names; use
underscores instead (`find-dialog` &rarr; `find_dialog`).
