...
both macros and translation table entries (see "[Key Binding](31)"
...
### Some notes on argument types above
  ...
  - `search-type`  
    Either "literal", "case", "word", "caseWord", "regex", or
    "regexNoCase".
...
## Window Preferences Actions
  ...
  - `set_em_tab_dist( em-tab-distance )`  
    ...
    1000.
  - `set_fonts( font-name, italic-font-name, bold-font-name,
...
supplied.
  ...
  - `raise_window([relative-window] | [positive-index] |
    [negative-index] [, "focus" | "nofocus"])`  
    ...
    Entries](30)") whether or not the input focus is requested.
