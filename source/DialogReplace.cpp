...
void DialogReplace::keyPressEvent(QKeyEvent *event) {
	...
	if(ui.textReplace->hasFocus()) {
		...
		if (index == 0) {
            ...
            ui.textReplace->setText(SearchReplaceHistory[historyIndex(index)].replace);
		}
...
void DialogReplace::on_buttonFind_clicked() {
    ...
    if (historyIndex(1) != -1 && SearchReplaceHistory[historyIndex(1)].search == searchString) {
        SearchReplaceHistory[historyIndex(1)].replace = replaceString;
	}
