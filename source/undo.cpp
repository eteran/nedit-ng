...
void Undo(WindowInfo *window) {
	...
	BufReplace(window->buffer, undo->startPos, undo->endPos, (undo->oldText != nullptr ? undo->oldText : ""));
...
void Redo(WindowInfo *window) {
	...
	BufReplace(window->buffer, redo->startPos, redo->endPos, (redo->oldText != nullptr ? redo->oldText : ""));
