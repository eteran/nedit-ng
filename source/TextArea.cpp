...
void TextArea::TextInsertAtCursorEx(view::string_view chars, bool allowPendingDelete, bool allowWrap) {
    ...
    int cursorPos = replaceSel ? buffer_->BufGetPrimary().start : cursorPos_;
...
int TextArea::startOfWord(int pos) {
	...
	if (c == ' ' || c == '\t') {
        ...
        if (!buffer_->BufSearchBackwardEx(pos, view::string_view(delimiters.data(), delimiters.size()), &startPos))
			return 0;
	}
...
int TextArea::endOfWord(int pos) {
	...
	if (c == ' ' || c == '\t') {
        ...
        if (!buffer_->BufSearchForwardEx(pos, view::string_view(delimiters.data(), delimiters.size()), &endPos))
			return buffer_->BufGetLength();
	}
	return endPos;
}
