...
static int insertEx(textBuffer *buf, int pos, const std::string &text);
static void deleteRange(textBuffer *buf, int start, int end);
static void deleteRect(textBuffer *buf, int start, int end, int rectStart, int rectEnd, int *replaceLen, int *endPos);
static void insertCol(textBuffer *buf, int column, int startPos, const char *insText, int *nDeleted, int *nInserted, int *endPos);
...
static void overlayRect(textBuffer *buf, int startPos, int rectStart, int rectEnd, const char *insText, int *nDeleted, int *nInserted, int *endPos);
static void overlayRectEx(textBuffer *buf, int startPos, int rectStart, int rectEnd, const std::string &insText, int *nDeleted, int *nInserted, int *endPos);
...
static void redisplaySelection(textBuffer *buf, Selection *oldSelection, Selection *newSelection);
...
static void updateSelections(textBuffer *buf, int pos, int nDeleted, int nInserted);
...
static void replaceSelectedEx(textBuffer *buf, Selection *sel, const std::string &text);
static void addPadding(char *string, int startIndent, int toIndent, int tabDist, int useTabs, char nullSubsChar, int *charsAdded);
static int searchForward(textBuffer *buf, int startPos, char searchChar, int *foundPos);
static int searchBackward(textBuffer *buf, int startPos, char searchChar, int *foundPos);
...
static void findRectSelBoundariesForCopy(textBuffer *buf, int lineStartPos, int rectStart, int rectEnd, int *selStart, int *selEnd);
...
static std::string realignTabsEx(const std::string &text, int origIndent, int newIndent, int tabDist, int useTabs, char nullSubsChar, int *newLength);
static std::string expandTabsEx(const std::string &text, int startIndent, int tabDist, char nullSubsChar, int *newLen);
static std::string unexpandTabsEx(const std::string &text, int startIndent, int tabDist, char nullSubsChar, int *newLen);

static const char *ControlCodeTable[32] = {"nul", "soh", "stx", "etx", "eot", "enq", "ack", "bel", "bs",  "ht", "nl",  "vt",  "np", "cr", "so", "si",
...
char *BufGetAll(textBuffer *buf) {
	...
	memcpy(&text[buf->gapStart], &buf->buf[buf->gapEnd], buf->length - buf->gapStart);
	text[buf->length] = '\0';
	return text;
}
...
void BufSetAll(textBuffer *buf, const char *text) {
	...
	{
		int i;
		for (i = buf->gapStart; i < buf->gapEnd; i++)
			buf->buf[i] = '.';
	}
#endif
...
void BufSetAllEx(textBuffer *buf, const std::string &text) {
	...
	memcpy(buf->buf, &text[0], buf->gapStart);
	memcpy(&buf->buf[buf->gapEnd], &text[buf->gapStart], length - buf->gapStart);
#ifdef PURIFY
	{
		int i;
...
void BufCopyFromBuf(textBuffer *fromBuf, textBuffer *toBuf, int fromStart, int fromEnd, int toPos) {
	...
	if (fromEnd <= fromBuf->gapStart) {
		...
		memcpy(&toBuf->buf[toPos], &fromBuf->buf[fromStart + (fromBuf->gapEnd - fromBuf->gapStart)], length);
	} else {
		part1Length = fromBuf->gapStart - fromStart;
		memcpy(&toBuf->buf[toPos], &fromBuf->buf[fromStart], part1Length);
		memcpy(&toBuf->buf[toPos + part1Length], &fromBuf->buf[fromBuf->gapEnd], length - part1Length);
	}
...
void BufInsertCol(textBuffer *buf, int column, int startPos, const char *text, int *charsInserted, int *charsDeleted) {
	...
	insertCol(buf, column, lineStartPos, text, &insertDeleted, &nInserted, &buf->cursorPosHint);
...
void BufOverlayRect(textBuffer *buf, int startPos, int rectStart, int rectEnd, const char *text, int *charsInserted, int *charsDeleted) {
	...
	callPreDeleteCBs(buf, lineStartPos, nDeleted);
	deletedText = BufGetRange(buf, lineStartPos, lineStartPos + nDeleted);
	overlayRect(buf, lineStartPos, rectStart, rectEnd, text, &insertDeleted, &nInserted, &buf->cursorPosHint);
	if (nDeleted != insertDeleted)
...
void BufOverlayRectEx(textBuffer *buf, int startPos, int rectStart, int rectEnd, const std::string &text, int *charsInserted, int *charsDeleted) {
	...
	lineStartPos = BufStartOfLine(buf, startPos);
	if (rectEnd == -1)
		rectEnd = rectStart + textWidthEx(text, buf->tabDist, buf->nullSubsChar);
	lineStartPos = BufStartOfLine(buf, startPos);
	nDeleted = BufEndOfLine(buf, BufCountForwardNLines(buf, startPos, nLines)) - lineStartPos;
	callPreDeleteCBs(buf, lineStartPos, nDeleted);
	std::string deletedText = BufGetRangeEx(buf, lineStartPos, lineStartPos + nDeleted);
...
void BufReplaceRect(textBuffer *buf, int start, int end, int rectStart, int rectEnd, const char *text) {
	...
	if (insText) {
		insertCol(buf, rectStart, start, insText, &insertDeleted, &insertInserted, &buf->cursorPosHint);
		XtFree(insText);
	} else
		insertCol(buf, rectStart, start, text, &insertDeleted, &insertInserted, &buf->cursorPosHint);
...
void BufReplaceRectEx(textBuffer *buf, int start, int end, int rectStart, int rectEnd, const std::string &text) {
	...
	if (insText) {
		...
		insertColEx(buf, rectStart, start, text, &insertDeleted, &insertInserted, &buf->cursorPosHint);
...
void BufRemoveRect(textBuffer *buf, int start, int end, int rectStart, int rectEnd) {
	...
	callPreDeleteCBs(buf, start, end - start);
	deletedText = BufGetRange(buf, start, end);
	deleteRect(buf, start, end, rectStart, rectEnd, &nInserted, &buf->cursorPosHint);
	callModifyCBs(buf, start, end - start, nInserted, 0, deletedText);
	XtFree(deletedText);
}
...
void BufClearRect(textBuffer *buf, int start, int end, int rectStart, int rectEnd) {
	...
	for (i = 0; i < nLines; i++)
		newlineString[i] = '\n';
	newlineString[i] = '\0';
	BufOverlayRect(buf, start, rectStart, rectEnd, newlineString, nullptr, nullptr);
...
int BufGetEmptySelectionPos(textBuffer *buf, int *start, int *end, int *isRect, int *rectStart, int *rectEnd) {
	return getSelectionPos(&buf->primary, start, end, isRect, rectStart, rectEnd) || buf->primary.zeroWidth;
}
...
void BufReplaceSelectedEx(textBuffer *buf, const std::string &text) {
	replaceSelectedEx(buf, &buf->primary, text);
}
...
int BufGetSecSelectPos(textBuffer *buf, int *start, int *end, int *isRect, int *rectStart, int *rectEnd) {
	return getSelectionPos(&buf->secondary, start, end, isRect, rectStart, rectEnd);
}
...
void BufReplaceSecSelectEx(textBuffer *buf, const std::string &text) {
	replaceSelectedEx(buf, &buf->secondary, text);
}
...
int BufGetHighlightPos(textBuffer *buf, int *start, int *end, int *isRect, int *rectStart, int *rectEnd) {
	return getSelectionPos(&buf->highlight, start, end, isRect, rectStart, rectEnd);
}
...
int BufGetExpandedChar(const textBuffer *buf, const int pos, const int indent, char *outStr) {
	return BufExpandCharacter(BufGetCharacter(buf, pos), indent, outStr, buf->tabDist, buf->nullSubsChar);
}
...
** equal in length to MAX_EXP_CHAR_LEN
*/
int BufExpandCharacter(const char c, const int indent, char *outStr, const int tabDist, const char nullSubsChar) {
	int i, nSpaces;
...
int BufCmp(textBuffer *buf, int pos, int len, const char *cmpText) {
	...
	if (posEnd <= buf->gapStart) {
		...
		return (strncmp(&buf->buf[pos + (buf->gapEnd - buf->gapStart)], cmpText, len));
	} else {
		...
		return (strncmp(&buf->buf[buf->gapEnd], &cmpText[part1Length], len - part1Length));
	}
}
...
int BufCmpEx(textBuffer *buf, int pos, int len, const std::string &cmpText) {
	...
	if (posEnd <= buf->gapStart) {
		return (strncmp(&(buf->buf[pos]), cmpText.c_str(), len));
	} else if (pos >= buf->gapStart) {
		return (strncmp(&buf->buf[pos + (buf->gapEnd - buf->gapStart)], cmpText.c_str(), len));
	} else {
		part1Length = buf->gapStart - pos;
		result = strncmp(&buf->buf[pos], cmpText.c_str(), part1Length);
		if (result) {
			return (result);
		}
		return (strncmp(&buf->buf[buf->gapEnd], &cmpText[part1Length], len - part1Length));
...
static int insertEx(textBuffer *buf, int pos, const std::string &text) {
	...
	/* Insert the new text (pos now corresponds to the start of the gap) */
	memcpy(&buf->buf[pos], &text[0], length);
	buf->gapStart += length;
	buf->length += length;
	updateSelections(buf, pos, 0, length);
...
static void insertCol(textBuffer *buf, int column, int startPos, const char *insText, int *nDeleted, int *nInserted, int *endPos) {
	...
	expText = expandTabs(replText, 0, buf->tabDist, buf->nullSubsChar, &expReplLen);
	XtFree(replText);
	XtFree(expText);
	expText = expandTabs(insText, 0, buf->tabDist, buf->nullSubsChar, &expInsLen);
	XtFree(expText);
	...
	while (True) {
		...
		insPtr += len;
		insertColInLine(line, insLine, column, insWidth, buf->tabDist, buf->useTabs, buf->nullSubsChar, outPtr, &len, &endOffset);
		XtFree(line);
		XtFree(insLine);
...
static void deleteRect(textBuffer *buf, int start, int end, int rectStart, int rectEnd, int *replaceLen, int *endPos) {
	...
	while (lineStart <= buf->length && lineStart <= end) {
		lineEnd = BufEndOfLine(buf, lineStart);
		line = BufGetRange(buf, lineStart, lineEnd);
		deleteRectFromLine(line, rectStart, rectEnd, buf->tabDist, buf->useTabs, buf->nullSubsChar, outPtr, &len, &endOffset);
		XtFree(line);
		outPtr += len;
		*outPtr++ = '\n';
		lineStart = lineEnd + 1;
...
static void overlayRect(textBuffer *buf, int startPos, int rectStart, int rectEnd, const char *insText, int *nDeleted, int *nInserted, int *endPos) {
	...
	while (True) {
		...
		insLine = copyLine(insPtr, &len);
		insPtr += len;
		overlayRectInLine(line, insLine, rectStart, rectEnd, buf->tabDist, buf->useTabs, buf->nullSubsChar, outPtr, &len, &endOffset);
		XtFree(line);
		XtFree(insLine);
		for (c = outPtr + len - 1; c > outPtr && (*c == ' ' || *c == '\t'); c--)
			len--;
...
static void overlayRectEx(textBuffer *buf, int startPos, int rectStart, int rectEnd, const std::string &insText, int *nDeleted, int *nInserted, int *endPos) {
	...
	std::string expText = expandTabsEx(insText, 0, buf->tabDist, buf->nullSubsChar, &expInsLen);
...
static void replaceSelectedEx(textBuffer *buf, Selection *sel, const std::string &text) {
	...
	/* Do the appropriate type of replace */
	if (isRect)
		BufReplaceRectEx(buf, start, end, rectStart, rectEnd, text);
	else
		BufReplaceEx(buf, start, end, text);
...
static void moveGap(textBuffer *buf, int pos) {
	...
	if (pos > buf->gapStart)
		memmove(&buf->buf[buf->gapStart], &buf->buf[buf->gapEnd], pos - buf->gapStart);
	else
...
static void reallocateBuf(textBuffer *buf, int newGapStart, int newGapLen) {
	...
	if (newGapStart <= buf->gapStart) {
		...
		memcpy(&newBuf[newGapEnd], &buf->buf[newGapStart], buf->gapStart - newGapStart);
		memcpy(&newBuf[newGapEnd + buf->gapStart - newGapStart], &buf->buf[buf->gapEnd], buf->length - buf->gapStart);
	} else { /* newGapStart > buf->gapStart */
		memcpy(newBuf, buf->buf, buf->gapStart);
		memcpy(&newBuf[buf->gapStart], &buf->buf[buf->gapEnd], newGapStart - buf->gapStart);
		memcpy(&newBuf[newGapEnd], &buf->buf[buf->gapEnd + newGapStart - buf->gapStart], buf->length - newGapStart);
	}
	XtFree(buf->buf);
	buf->buf = newBuf;
...
static std::string expandTabsEx(const std::string &text, int startIndent, int tabDist, char nullSubsChar, int *newLen) {
	...
	for (char ch : text) {
		if (ch == '\t') {
			...
			char *temp_ptr = temp;
			len = BufExpandCharacter(ch, indent, temp, tabDist, nullSubsChar);
...
static std::string unexpandTabsEx(const std::string &text, int startIndent, int tabDist, char nullSubsChar, int *newLen) {
	...
	for (auto c = text.begin(); c != text.end();) {
		if (*c == ' ') {
			int len = BufExpandCharacter('\t', indent, expandedChar, tabDist, nullSubsChar);
			if (len >= 3 && !strncmp(&*c, expandedChar, len)) {
				c += len;
				*outPtr++ = '\t';
				indent += len;
