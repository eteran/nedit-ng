...
typedef struct {
	...
	QString sel1;             /* selection from list 1 */
	QString sel2;             /* selection from list 2 */
	QString sel3;             /* selection from list 3 */
...
static void enableSample(xfselControlBlkType *ctrlBlk, Bool turn_on, XmFontList *fontList);
...
char *FontSel(Widget parent, int showPropFonts, const char *currFont, Pixel sampleFG, Pixel sampleBG) {
	Widget dialog, form, okButton, cancelButton;
	Widget styleList, sizeList, fontName, fontList;
	Widget sizeToggle, propFontToggle = nullptr, dispField;
	...
	ctrlBlk.fontData = XListFonts(XtDisplay(parent), "-*-*-*-*-*-*-*-*-*-*-*-*-*-*", MAX_NUM_FONTS, &ctrlBlk.numFonts);
	...
	form = XtCreateWidget("Font Selector", xmFormWidgetClass, dialog, args, n);
	...
	n++;
	okButton = XtCreateManagedWidget("OK", xmPushButtonWidgetClass, form, args, n);
	...
	XtSetArg(args[n], XmNheight, 28);
	n++;
	cancelButton = XtCreateManagedWidget("Cancel", xmPushButtonWidgetClass, form, args, n);
	...
	n++;
	fontName = XtCreateManagedWidget("fontname", xmTextWidgetClass, form, args, n);
	...
	XtSetArg(args[n], XmNtopOffset, 1);
	n++;
	nameLabel = XtCreateManagedWidget("Font Name:", xmLabelWidgetClass, form, args, n);
	...
	n++;
	XtSetArg(args[n], XmNvalue, "ABCDEFGHIJKLMNOPQRSTUVWXYZ abcdefghijklmnopqrstuvwxyz 0123456789");
	n++;
	...
	n++;
	sizeToggle = XtCreateManagedWidget("sizetoggle", xmToggleButtonWidgetClass, form, args, n);
	XmStringFree(tempStr);
	...
	if (showPropFonts != ONLY_FIXED) {
		...
		XtSetArg(args[n], XmNtopOffset, 1);
		n++;
		propFontToggle = XtCreateManagedWidget("propfonttoggle", xmToggleButtonWidgetClass, form, args, n);
		XmStringFree(tempStr);
	}
	...
	XtSetArg(args[n], XmNleftPosition, 1);
	n++;
	nameLabel = XtCreateManagedWidget("Font:", xmLabelWidgetClass, form, args, n);
	XmStringFree(tempStr);
	...
	XtVaSetValues(nameLabel, XmNuserData, fontList, nullptr);
	...
	ctrlBlk.sel1 = QString();
	ctrlBlk.sel2 = QString();
	ctrlBlk.sel3 = QString();
	ctrlBlk.fontName = nullptr;
...
static void setupScrollLists(int dontChange, xfselControlBlkType ctrlBlk) {
	...
	for (i = 0; i < ctrlBlk.numFonts && i < MAX_ENTRIES_IN_LIST; i++) {
		if ((dontChange != FONT) && (styleMatch(&ctrlBlk, ctrlBlk.fontData[i])) && (sizeMatch(&ctrlBlk, ctrlBlk.fontData[i])) && ((ctrlBlk.showPropFonts == PREF_PROP) || (notPropFont(ctrlBlk.fontData[i])))) {
			getFontPart(ctrlBlk.fontData[i], buff1);
			addItemToList(itemBuf1, buff1, &itemCount1);
		}

		if ((dontChange != STYLE) && (fontMatch(&ctrlBlk, ctrlBlk.fontData[i])) && (sizeMatch(&ctrlBlk, ctrlBlk.fontData[i])) && ((ctrlBlk.showPropFonts == PREF_PROP) || (notPropFont(ctrlBlk.fontData[i])))) {
			getStylePart(ctrlBlk.fontData[i], buff1);
			addItemToList(itemBuf2, buff1, &itemCount2);
		}

		if ((dontChange != SIZE) && (fontMatch(&ctrlBlk, ctrlBlk.fontData[i])) && (styleMatch(&ctrlBlk, ctrlBlk.fontData[i])) && ((ctrlBlk.showPropFonts == PREF_PROP) || (notPropFont(ctrlBlk.fontData[i])))) {
	...
	if (dontChange != FONT) {
		...
		if (!ctrlBlk.sel1.isNull()) {
			XmStringFree(items[0]);
			items[0] = XmStringCreate(ctrlBlk.sel1.toLatin1().data(), XmSTRING_DEFAULT_CHARSET);
	...
	if (dontChange != STYLE) {
		...
		if (!ctrlBlk.sel2.isNull()) {
			XmStringFree(items[0]);
			items[0] = XmStringCreate(ctrlBlk.sel2.toLatin1().data(), XmSTRING_DEFAULT_CHARSET);
	...
	if (dontChange != SIZE) {
		for (i = 0; i < itemCount3; i++) {
			items[i] = XmStringCreate(itemBuf3[i], XmSTRING_DEFAULT_CHARSET);
			XtFree(itemBuf3[i]);
		}
		XmListDeleteAllItems(ctrlBlk.sizeList);
		XmListAddItems(ctrlBlk.sizeList, items, itemCount3, 1);
		if (!ctrlBlk.sel3.isNull()) {
			XmStringFree(items[0]);
			items[0] = XmStringCreate(ctrlBlk.sel3.toLatin1().data(), XmSTRING_DEFAULT_CHARSET);
...
static int styleMatch(xfselControlBlkType *ctrlBlk, const char *font) {
	...
	if (ctrlBlk->sel2 == QLatin1String(buff)) {
		return (TRUE);
	} else {
		return (FALSE);
	}
...
static int sizeMatch(xfselControlBlkType *ctrlBlk, const char *font) {
	...
	if (QLatin1String(buff) == ctrlBlk->sel3)
		return (TRUE);
	else
		return (FALSE);
}
...
static int fontMatch(xfselControlBlkType *ctrlBlk, const char *font) {
	...
	if (QLatin1String(buff) == ctrlBlk->sel1)
		return (TRUE);
	else
		return (FALSE);
}
...
static void propFontToggleAction(Widget widget, XtPointer controlBlock, XtPointer callData) {
	...
	auto ctrlBlk   = static_cast<xfselControlBlkType *>(controlBlock);
	auto call_data = static_cast<XmToggleButtonCallbackStruct *>(callData);
...
static void sizeToggleAction(Widget widget, XtPointer controlBlock, XtPointer callData) {
	...
	if (call_data->reason == XmCR_VALUE_CHANGED) {
		...
		bool makeSelection = !ctrlBlk->sel3.isNull();
...
static void fontAction(Widget widget, XtPointer controlBlock, XtPointer callData) {
	...
	auto ctrlBlk   = static_cast<xfselControlBlkType *>(controlBlock);
	auto call_data = static_cast<XmListCallbackStruct *>(callData);

	std::string sel = XmStringGetLtoREx(call_data->item, XmSTRING_DEFAULT_CHARSET);

	if(ctrlBlk->sel1.isNull()) {
		ctrlBlk->sel1 = QString::fromStdString(sel);
	} else {
		if (ctrlBlk->sel1 == QString::fromStdString(sel)) { /* Unselecting current selection */
			ctrlBlk->sel1 = QString();
	...
	if ((!ctrlBlk->sel1.isNull()) && (!ctrlBlk->sel2.isNull()) && (!ctrlBlk->sel3.isNull()))
		choiceMade(ctrlBlk);
	else {
		enableSample(ctrlBlk, False, nullptr);
		XmTextSetStringEx(ctrlBlk->fontNameField, "");
...
static void styleAction(Widget widget, XtPointer controlBlock, XtPointer callData) {
	...
	if(ctrlBlk->sel2.isNull()) {
		ctrlBlk->sel2 = QString::fromStdString(sel);
	} else {
		if (ctrlBlk->sel2 == QString::fromStdString(sel)) { /* unselecting current selection */
			ctrlBlk->sel2 = QString();
			XmListDeselectItem(widget, call_data->item);
		} else {
			ctrlBlk->sel2 = QString::fromStdString(sel);
...
static void sizeAction(Widget widget, XtPointer controlBlock, XtPointer callData) {
	...
	if(ctrlBlk->sel3.isNull()) {
		ctrlBlk->sel3 = QString::fromStdString(sel);
	} else {
		if (ctrlBlk->sel3 == QString::fromStdString(sel)) { /* unselecting current selection */
			ctrlBlk->sel3 = QString();
			XmListDeselectItem(widget, call_data->item);
		} else {
			ctrlBlk->sel3 = QString::fromStdString(sel);
...
static void startupFont(xfselControlBlkType *ctrlBlk, const char *font) {
	...
	if (i == 0) { /*  invalid font passed in at startup */
		XFreeFontNames(fontName);
		return;
	}
