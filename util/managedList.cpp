...
Widget CreateManagedList(Widget parent, const char *name, Arg *args, int argC, void **itemList, int *nItems, int maxItems, int nColumns, void *(*getDialogDataCB)(void *, int, int *, void *), void *getDialogDataArg,
	...
	return ManageListAndButtons(listW, deleteBtn, copyBtn, moveUpBtn, moveDownBtn, itemList, nItems, maxItems, getDialogDataCB, getDialogDataArg, setDialogDataCB, setDialogDataArg, freeItemCB);
}
...
static void copyCB(Widget w, XtPointer clientData, XtPointer callData) {
	...
	item = (*ml->getDialogDataCB)(ml->itemList[listPos - 2], True, &abort, ml->getDialogDataArg);
	if (abort)
		return;
